function simulate(model::Model,n::Int64,p::Float64)
    ...
    n=n-nb_maint
     ...
     for i in 1:length(diff(t))
         inc=[inc;reshape(rand(MvNormal([Model.mu1*diff(t)[i],mu2*diff(t)[i]],[sigma21*diff(t)[i] cov*diff(t)[i] ; cov*diff(t)[i] sigma22*diff(t)[i]])),1,2)]
     end
